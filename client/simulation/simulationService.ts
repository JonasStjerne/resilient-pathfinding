...
import { SimulationOptions, Stats } from './models'

export class simulationService {
  static getSimOptions() {
    const algoVersion = <'v0.1' | 'v0.2'>(
      (<HTMLInputElement>(
        document.querySelector('input[name="algo-version"]:checked')
      )).value
    )
    const riskFactor = Number(
      (<HTMLInputElement>document.getElementById('risk-factor-sim')).value,
    )
    const runCount = Number(
      (<HTMLInputElement>document.getElementById('iteration-count')).value,
    )
